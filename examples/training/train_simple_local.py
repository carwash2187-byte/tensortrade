#!/usr/bin/env python3
"""
Same demo as train_simple.py, but reads bundled CSV data instead of
downloading from CryptoDataDownload -- for offline/restricted-network runs.
"""
...
def main():
    print("="*70)
    print("TensorTrade Training Demo - Showing Wallet Balances (local data)")
    print("="*70)

    print("\nLoading BTC/USD data from examples/data/Coinbase_BTCUSD_1h.csv...")
    data = pd.read_csv("examples/data/Coinbase_BTCUSD_1h.csv", skiprows=1)
    ...
    print(f"Initial Cash:  ${initial_cash:,.2f} USD")
    print(f"Initial Asset: 0.00000000 BTC")
